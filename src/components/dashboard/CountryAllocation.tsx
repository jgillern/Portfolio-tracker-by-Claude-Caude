...
export function CountryAllocation() {
  ...
  const { countries, symbolCountryMap, isLoading } = useCountries(symbols, types, weights);

  if (!activePortfolio || instruments.length === 0) return null;

  const totalWeight = weights.reduce((a, b) => a + b, 0);

  const getInstrumentsForCountry = (countryName: string): AllocationInstrument[] => {
    const result: AllocationInstrument[] = [];
    instruments.forEach((instrument, idx) => {
      const country = symbolCountryMap.get(instrument.symbol) ?? 'N/A';
      if (country === countryName) {
        result.push({
          instrument,
          weight: totalWeight > 0 ? (weights[idx] / totalWeight) * 100 : 0,
        });
      }
    });
    return result;
  ...
  return (
    <>
          ...
          <>
            <div className="flex h-4 rounded-full overflow-hidden mb-4">
              {countries.map((item, i) => (
                <div
                  key={item.country}
                  className={`${COUNTRY_COLORS[i % COUNTRY_COLORS.length]} transition-all cursor-pointer hover:opacity-80`}
      ...
      <AllocationDetailModal
        ...
        instruments={selectedCountry ? getInstrumentsForCountry(selectedCountry) : []}
      />
    </>
  );
}
