      ...
      {isLoading ? (
        ...
        <div className="overflow-y-auto -mx-6 px-6 pb-6" style={{ maxHeight: 'calc(92vh - 120px)' }}>
          ...
          {(profile.website || profile.employees) && (
            <div className="mt-8 pt-5 border-t border-gray-200 dark:border-gray-700 flex items-center gap-5 flex-wrap text-sm text-gray-500 dark:text-gray-400">
              {profile.employees && (
                <div className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
                  </svg>
                  <span className="font-medium">{profile.employees.toLocaleString()} {t('profile.employees')}</span>
                </div>
              )}
              {profile.website && (
                <a href={profile.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-blue-500 hover:text-blue-600 font-medium transition-colors">
