...
function getDomain(articleUrl: string): string | null {
  try {
...
export function NewsCard({ article }: Props) {
  ...
  const logoUrl = domain ? `https://logo.clearbit.com/${domain}?size=96` : null;
  const [logoError, setLogoError] = useState(false);

  const showThumbnail = article.thumbnailUrl && !imgError;
  const showLogo = !showThumbnail && logoUrl && !logoError;
