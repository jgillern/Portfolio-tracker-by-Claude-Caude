...
export function EToroMetrics({ positions }: Props) {
  const { t } = useLanguage();

  // Filter positions that have Yahoo Finance symbols and compute weights
  const mappedPositions = positions.filter((p) => p.yahooSymbol);
  const symbols = mappedPositions.map((p) => p.yahooSymbol!);
  const totalAlloc = mappedPositions.reduce((s, p) => s + p.allocationPct, 0);
  const weights = mappedPositions.map((p) =>
    totalAlloc > 0 ? (p.allocationPct / totalAlloc) * 100 : 0
  );

  const { metrics, isLoading } = useMetrics(symbols, weights);

  if (mappedPositions.length === 0) return null;
